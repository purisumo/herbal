...
def train_model(label_file, herb_file):
    ...
    x_train_resized = [array_to_img(img, scale=False).resize((299, 299)) for img in x_train]
    x_test_resized = [array_to_img(img, scale=False).resize((299, 299)) for img in x_test]

    x_train = np.array([img_to_array(img) for img in x_train_resized])
    x_test = np.array([img_to_array(img) for img in x_test_resized])

    x_train = x_train.astype('float32')
    x_test = x_test.astype('float32')
    x_train = preprocess_input(x_train)  # Preprocess input for InceptionV3
    x_test = preprocess_input(x_test)    # Preprocess input for InceptionV3
    ...
    test_loss, test_acc = model.evaluate(x_test, y_test, verbose=2)
    print(test_acc)
    model.save("model_with_transfer_learning_inceptionv3.h5")
